...
class LIB_EXPORT Board
{
	...
	public:
		 ...
		 */
		virtual bool isRandomVariant() const = 0;
		/*!
		 ...
		 * \sa CrazyhouseBoard
		 */
		virtual bool variantHasDrops() const = 0;;
		/*!
		 * Returns true if the board accepts wall squares, else false.
		 * The default value is false.
		 */
		virtual bool variantHasWallSquares() const = 0;;
		/*!
		 ...
		 */
		virtual QList<Piece> reservePieceTypes() const = 0;;
		 ...
		 */
		virtual bool setFenString(const QString& fen) = 0;
		/*!
		 ...
		 */
		virtual Side upperCaseSide() const = 0;
		 ...
		 */
		virtual int reversibleMoveCount() const = 0;
		/*!
		 ...
		 * always returns 0.
		 */
		virtual int reserveCount(Piece piece) const = 0;
		/*! Converts \a piece into a piece symbol. */
		virtual QString pieceSymbol(Piece piece) const = 0;
		/*! Converts \a pieceSymbol into a Piece object. */
		virtual Piece pieceFromSymbol(const QString& pieceSymbol) const = 0;
		/*! Returns the internationalized name of \a pieceType. */
		virtual QString pieceString(int pieceType) const = 0;
		/*! Returns symbol for graphical representation of \a piece. */
		virtual QString representation(Piece piece) const = 0;
		 ...
		 * the board.
		 */
		virtual void makeMove(const Move& move, BoardTransition* transition = nullptr) = 0;
		 ...
		 * \sa moveFromString()
		 */
		virtual QString moveString(const Move& move, MoveNotation notation) = 0;;
		/*!
		 ...
		 * \sa moveString()
		 */
		virtual Move moveFromString(const QString& str) = 0;;
		/*!
		 ...
		 * \sa genericMove()
		 */
		virtual Move moveFromGenericMove(const GenericMove& move) const = 0;
		/*!
		 ...
		 */
		virtual Result tablebaseResult(unsigned int* dtm = nullptr) const = 0;

		virtual void setWidth(int wd) = 0;
	...
	protected:
		 ...
		 */
		virtual void setPieceType(int type,
				  const QString& name,
				  const QString& symbol,
				  unsigned movement = 0,
				  const QString & gsymbol = QString()) = 0;
		/*! Returns true if \a pieceType can move like \a movement. */
		//virtual bool pieceHasMovement(int pieceType, unsigned movement) const = 0;
		...
		virtual Square chessSquare(int index) const = 0;
		/*! Converts a string into a Square object. */
		virtual Square chessSquare(const QString& str) const = 0;
		/*! Converts a Square object into a square index. */
		virtual int squareIndex(const Square& square) const = 0;
		/*! Converts a string into a square index. */
		virtual int squareIndex(const QString& str) const = 0;
		/*! Converts a square index into a string. */
		virtual QString squareString(int index) const = 0;
		/*! Converts a Square object into a string. */
		virtual QString squareString(const Square& square) const = 0;
		...
		virtual Move moveFromLanString(const QString& str) = 0;
		...
		virtual int maxPieceSymbolLength() const = 0;
		 ...
		 * \sa legalMoves()
		 */
		virtual void generateMoves(QVarLengthArray<Move>& moves,
				   int pieceType = Piece::NoPiece) const = 0;
		/*!
		 ...
		 * \sa generateMoves()
		 */
		virtual void generateDropMoves(QVarLengthArray<Move>& moves, int pieceType) const = 0;
		/*!
		 ...
		 */
		virtual void generateHoppingMoves(int sourceSquare,
					  const QVarLengthArray<int>& offsets,
					  QVarLengthArray<Move>& moves) const = 0;
		/*!
		 ...
		 * \note The generated \a moves include captures
		 */
		virtual void generateSlidingMoves(int sourceSquare,
					  const QVarLengthArray<int>& offsets,
					  QVarLengthArray<Move>& moves) const = 0;
		/*!
		...
		/*! Returns the piece at \a square. */
		virtual Piece pieceAt(int square) const = 0;
		/*!
		 ...
		 */
		virtual void setSquare(int square, Piece piece) = 0;
		/*! Returns the last move made in the game. */
		virtual const Move& lastMove() const = 0;
		/*!
		 ...
		 */
		virtual int reserveType(int pieceType) const = 0;
		/*! Adds \a count pieces of type \a piece to the reserve. */
		virtual void addToReserve(const Piece& piece, int count = 1) = 0;
		/*! Removes a piece of type \a piece from the reserve. */
		virtual void removeFromReserve(const Piece& piece) = 0;
