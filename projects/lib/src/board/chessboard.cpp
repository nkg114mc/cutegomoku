...
QDebug operator<<(QDebug dbg, const ChessBoard* board)
{/*
	QString str = "FEN: " + board->fenString() + '\n';
	str += ChessBoard::tr("Zobrist key") + ": 0x" +
...
ChessBoard::ChessBoard(Zobrist* zobrist)
	: Board(),
	  m_initialized(false),
	  m_width(0),
	  m_height(0),
...
bool ChessBoard::isRandomVariant() const
{
	return false;
}

bool ChessBoard::variantHasDrops() const
{
	return false;
}

bool ChessBoard::variantHasWallSquares() const
{
	return false;
}

QList<Piece> ChessBoard::reservePieceTypes() const
{
	return QList<Piece>();
}

ChessBoard::CoordinateSystem ChessBoard::coordinateSystem() const
{
	return NormalCoordinates;
}

Side ChessBoard::upperCaseSide() const
{
	return Side::White;
}

Piece ChessBoard::pieceAt(const Square& square) const
{
...
int ChessBoard::maxPieceSymbolLength() const
{
	return m_maxPieceSymbolLength;
}

void ChessBoard::setPieceType(int type,
			 const QString& name,
...
QString ChessBoard::pieceSymbol(Piece piece) const
{
	int type = piece.type();
...
Piece ChessBoard::pieceFromSymbol(const QString& pieceSymbol) const
{
...
QString ChessBoard::pieceString(int pieceType) const
{
...
QString ChessBoard::representation(Piece piece) const
{
	int type = piece.type();
	if (type <= 0 || type >= m_pieceData.size())
		return QString();
...
int ChessBoard::reserveType(int pieceType) const
{
	return pieceType;
}

int ChessBoard::reserveCount(Piece piece) const
{
	if (!piece.isValid()
...
void ChessBoard::addToReserve(const Piece& piece, int count)
{
...
void ChessBoard::removeFromReserve(const Piece& piece)
{
	Q_ASSERT(piece.isValid());
...
Square ChessBoard::chessSquare(int index) const
{
...
bool ChessBoard::isValidSquare(const Chess::Square& square) const
{
	if (!square.isValid()
...
QString ChessBoard::squareString(int index) const
{
	return squareString(chessSquare(index));
}

QString ChessBoard::squareString(const Square& square) const
{
	if (!square.isValid())
		return QString();
...
Square ChessBoard::chessSquare(const QString& str) const
{
	if (str.length() < 2)
		return Square();
...
int ChessBoard::squareIndex(const QString& str) const
{
	return squareIndex(chessSquare(str));
}

QString ChessBoard::lanMoveString(const Move& move)
{
	QString str;
...
QString ChessBoard::moveString(const Move& move, MoveNotation notation)
{
...
Move ChessBoard::moveFromLanString(const QString& istr)
{
	QString str(istr);
...
Move ChessBoard::moveFromString(const QString& str)
{
...
Move ChessBoard::moveFromGenericMove(const GenericMove& move) const
{
...
GenericMove ChessBoard::genericMove(const Move& move) const
{
...
QStringList ChessBoard::pieceList(Side side) const
{
	QStringList list;
...
QString ChessBoard::fenString(FenNotation notation) const
{
	QString fen;
...
bool ChessBoard::setFenString(const QString& fen)
{
...
void ChessBoard::makeMove(const Move& move, BoardTransition* transition)
{
	...
	std::cout << "call ChessBoard::makeMove" << std::endl;
}
...
void ChessBoard::generateMoves(QVarLengthArray<Move>& moves, int pieceType) const
{
	Q_ASSERT(!m_side.isNull());
...
void ChessBoard::generateDropMoves(QVarLengthArray<Move>& moves, int pieceType) const
{
...
void ChessBoard::generateHoppingMoves(int sourceSquare,
...
void ChessBoard::generateSlidingMoves(int sourceSquare,
				 const QVarLengthArray<int>& offsets,
				 QVarLengthArray<Move>& moves) const
{
...
bool ChessBoard::moveExists(const Move& move) const
{
	Q_ASSERT(!move.isNull());
...
int ChessBoard::captureType(const Move& move) const
{
	Q_ASSERT(!move.isNull());
...
bool ChessBoard::vIsLegalMove(const Move& move)
{
	Q_ASSERT(!move.isNull());
...
int ChessBoard::repeatCount() const
{
	if (plyCount() < 4)
		return 0;
...
bool ChessBoard::isRepetition(const Chess::Move& move)
{
	Q_ASSERT(!move.isNull());
...
QVector<Move> ChessBoard::legalMoves()
{
	QVarLengthArray<Move> moves;
...
Result ChessBoard::tablebaseResult(unsigned int* dtm) const
{
	Q_UNUSED(dtm);
	return Result();
}
...
extern LIB_EXPORT QDebug operator<<(QDebug dbg, const ChessBoard* board);

int ChessBoard::arraySize() const
{
	return m_squares.size();
}

Side ChessBoard::sideToMove() const
{
	return m_side;
}

Side ChessBoard::startingSide() const
{
	return m_startingSide;
}

QString ChessBoard::startingFenString() const
{
	return m_startingFen;
}

quint64 ChessBoard::key() const
{
	return m_key;
}

void ChessBoard::xorKey(quint64 key)
{
	m_key ^= key;
}

Piece ChessBoard::pieceAt(int square) const
{
	return m_squares[square];
}

void ChessBoard::setSquare(int square, Piece piece)
{
...
int ChessBoard::plyCount() const
{
	return m_moveHistory.size();
}

const Move& ChessBoard::lastMove() const
{
	return m_moveHistory.last().move;
}

bool ChessBoard::pieceHasMovement(int pieceType, unsigned movement) const
{
...
void ChessBoard::setWidth(int wd) {
	Q_ASSERT(wd > 0 && wd < 32);
	//m_width = wd; // we do not allow to change the board size
}

void ChessBoard::setHeight(int ht) {
	Q_ASSERT(ht > 0 && ht < 32);
	//m_height = ht; // we do not allow to change the board size
}
