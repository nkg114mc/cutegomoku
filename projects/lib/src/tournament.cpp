...
void Tournament::setBoardSize(int sz)
{
	Q_ASSERT(sz > 4 && sz < 32);
	m_customizedBoardSize = sz;
}
...
void Tournament::startGame(TournamentPair* pair)
{
	...
	// for custumized board size
	if (m_customizedBoardSize != -1) {
		board->setSize(m_customizedBoardSize);
	}
